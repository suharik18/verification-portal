...
class ValidateIssuer implements PipeInterface
{
    public function handle($document, $next)
    {
        ...
        $dnsLocation = Arr::get($document, 'data.issuer.identityProof.location');
        $isKeyCorrect = DnsHelper::loadGoogleDnsList($dnsLocation)
            ->contains(fn ($value) => str_contains($value['data'], $key));
